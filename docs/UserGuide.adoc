...
You can use the `help` command to access the *User Guide* in *PlanWithEase*.

Format: `help`

[TIP]
====
Alternatively, you can access the `help` page via kbd:[F1] or from the menu bar, as shown below.
...
Lists all the commands that you have entered in reverse chronological order (from most recent to earliest). +
Format: `history`
...
Undoable commands: those commands that modify the contents of PlanWithEase's data.

The table below shows commands that is undoable.
...
==== Clearing modules : `clear`

This command allows you to remove all modules from the section specified. +
If no parameters are given, all modules will be removed from the *module list*, *requirement categories* and *degree
plan*.

Format: `clear [requirement] [planner]`

[TIP]
====
If you want to remove all *requirement categories* or *degree plan* quickly, use this command instead!
====
...
==== Listing degree planners: `planner_list`
